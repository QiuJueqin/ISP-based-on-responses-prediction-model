...
if (max(eta)/min(eta) > 10 | max(rho)/min(rho) > 10)
  if (nargin < 5)
    ...
    for k = ni:ni:n
      text(rho(k),eta(k),num2str(reg_param(k)),'Interpreter','latex','FontSize',12);
    end
...
xlabel('$\textrm{Residual Error } \left\|{\mathbf{L''}_{\textrm{rel}} \cdot \mathbf{S} - \mathbf{r}}\right\|_2$','Interpreter','latex','FontSize',14);
if (ps==1)
  ylabel('solution norm || x ||_2')
else
%   ylabel('solution semi-norm || L x ||_2')
  ylabel('$\textrm{Regularized Error }{\left\| {\mathcal{R}{\mathbf{S}}} \right\|_2}$','Interpreter','latex','FontSize',14,'Rotation',90);
end
title('L-curve')